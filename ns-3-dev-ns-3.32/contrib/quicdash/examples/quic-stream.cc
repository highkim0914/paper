...
// - QUIC Stream server and user-defined number of clients connected with an AP
// - WiFi connection
...
#include "ns3/quic-stream-helper.h"
#include "ns3/quic-stream-interface.h"
...
NS_LOG_COMPONENT_DEFINE ("QuicStreamExample");
...
//
  ...
  LogComponentEnable ("QuicStreamExample", LOG_LEVEL_INFO);
  // LogComponentEnable ("QuicStreamClientApplication", LOG_LEVEL_INFO);
  // LogComponentEnable ("QuicStreamServerApplication", LOG_LEVEL_INFO);
  
  // LogComponentEnable ("QuicSocketBase", LOG_LEVEL_ERROR);
  // LogComponentEnable ("QuicStreamBase", LOG_LEVEL_ERROR);
  // LogComponentEnable ("QuicSocketTxBuffer", LOG_LEVEL_ERROR);
  // LogComponentEnable ("QuicSocketRxBuffer", LOG_LEVEL_ERROR);
  // LogComponentEnable ("QuicStreamTxBuffer", LOG_LEVEL_ERROR);
  // LogComponentEnable ("QuicStreamRxBuffer", LOG_LEVEL_ERROR);
  // LogComponentEnable ("QuicStreamBase", LOG_LEVEL_WARN);
  // LogComponentEnable ("QuicSocketBase", LOG_LEVEL_WARN);
  // LogComponentEnable ("QuicSocketTxBuffer", LOG_LEVEL_WARN);
  // LogComponentEnable ("QuicSocketRxBuffer", LOG_LEVEL_WARN);
  // LogComponentEnable ("QuicStreamTxBuffer", LOG_LEVEL_WARN);
  // LogComponentEnable ("QuicStreamRxBuffer", LOG_LEVEL_WARN);

  // LogComponentEnable ("QuicSocketTxBuffer", LOG_LEVEL_FUNCTION);
  // LogComponentEnable ("QuicSocketRxBuffer", LOG_LEVEL_FUNCTION);
  // LogComponentEnable ("QuicStreamTxBuffer", LOG_LEVEL_FUNCTION);
  // LogComponentEnable ("QuicStreamRxBuffer", LOG_LEVEL_FUNCTION);
  ...
  Config::SetDefault("ns3::QuicSocketBase::SocketSndBufSize", UintegerValue (buffersize));
  Config::SetDefault("ns3::QuicSocketBase::SocketRcvBufSize", UintegerValue (buffersize));
  Config::SetDefault("ns3::QuicSocketBase::LegacyCongestionControl", BooleanValue(true));
  

  Config::SetDefault("ns3::QuicStreamBase::StreamSndBufSize", UintegerValue (buffersize));
  Config::SetDefault("ns3::QuicStreamBase::StreamRcvBufSize", UintegerValue (buffersize));

  // don't fragment tag
  Config::SetDefault("ns3::UdpSocket::MtuDiscover", BooleanValue(true));
  ...
  if(simulationId == 5)
    p2p.SetDeviceAttribute ("DataRate", StringValue ("50000kb/s")); // This must not be more than the maximum throughput in 802.11n
  else if (simulationId == 10)
    p2p.SetDeviceAttribute ("DataRate", StringValue ("150000kb/s")); // This must not be more than the maximum throughput in 802.11n
  ...
  else if (simulationId == 20)
    p2p.SetDeviceAttribute ("DataRate", StringValue ("10000kb/s")); // This must not be more than the maximum throughput in 802.11n
  ...
  /* Internet stack */
  QuicHelper stack;
  stack.InstallQuic (networkNodes);
...
//////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  const char * mylogsDir = quicDashLogDirectory.c_str();
  mkdir (mylogsDir, 0775);
  std::string algodirstr (quicDashLogDirectory +  adaptationAlgo );  
  const char * algodir = algodirstr.c_str();
  mkdir (algodir, 0775);
  std::string dirstr (quicDashLogDirectory + adaptationAlgo + "/" + ToString (numberOfClients) + "/");
  const char * dir = dirstr.c_str();
  mkdir(dir, 0775);

  std::ofstream clientPosLog;
  std::string clientPos = quicDashLogDirectory + "/" + adaptationAlgo + "/" + ToString (numberOfClients) + "/" + "sim" + ToString (simulationId) + "_"  + "clientPos.txt";
  ...
  // p2p.EnablePcapAll ("quicstream-p2p-", true);
  // wifiPhy.EnablePcapAll ("quicstream-wifi-", true);



  /* Install QUIC Receiver on the access point */
  QuicStreamServerHelper serverHelper (port);
  ApplicationContainer serverApp = serverHelper.Install (serverNode);
  serverApp.Start (Seconds (1.0));
  /* Install QUIC/TCP/UDP Transmitter on the station */
  QuicStreamClientHelper clientHelper (serverAddress, port);
